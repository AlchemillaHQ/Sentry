...
func (m *MockPushSender) Send(ctx context.Context, platform, token, callID, callerURI, callerName string) error {
	return m.Called(ctx, platform, token, callID, callerURI, callerName).Error(0)
}
...
func (m *MockQuerier) CreatePendingCall(ctx context.Context, arg db.CreatePendingCallParams) error { return m.Called(ctx, arg).Error(0) }
func (m *MockQuerier) DeletePendingCall(ctx context.Context, callID string) error { return m.Called(ctx, callID).Error(0) }
func (m *MockQuerier) DeleteDeviceByID(ctx context.Context, deviceID string) error { return m.Called(ctx, deviceID).Error(0) }
func (m *MockQuerier) GetDeviceByB2BUASIPUser(ctx context.Context, b2buaSipUser string) (db.Device, error) {
	args := m.Called(ctx, b2buaSipUser); return args.Get(0).(db.Device), args.Error(1)
}
func (m *MockQuerier) GetDeviceByID(ctx context.Context, deviceID string) (db.Device, error) {
	args := m.Called(ctx, deviceID); return args.Get(0).(db.Device), args.Error(1)
}
func (m *MockQuerier) GetDevicesByUpstreamUser(ctx context.Context, upstreamUser string) ([]db.Device, error) {
	args := m.Called(ctx, upstreamUser); return args.Get(0).([]db.Device), args.Error(1)
}
func (m *MockQuerier) GetPendingCall(ctx context.Context, callID string) (db.PendingCall, error) {
	args := m.Called(ctx, callID); return args.Get(0).(db.PendingCall), args.Error(1)
}
func (m *MockQuerier) GetSetting(ctx context.Context, key string) ([]byte, error) {
	args := m.Called(ctx, key); return args.Get(0).([]byte), args.Error(1)
}
...
func (m *MockQuerier) UpsertDevice(ctx context.Context, arg db.UpsertDeviceParams) error { return m.Called(ctx, arg).Error(0) }
func (m *MockQuerier) CreateUser(ctx context.Context, arg db.CreateUserParams) error { return m.Called(ctx, arg).Error(0) }
func (m *MockQuerier) DeleteUser(ctx context.Context, username string) error { return m.Called(ctx, username).Error(0) }
func (m *MockQuerier) GetUser(ctx context.Context, username string) (db.User, error) {
	args := m.Called(ctx, username); return args.Get(0).(db.User), args.Error(1)
}
func (m *MockQuerier) ListUsers(ctx context.Context) ([]db.ListUsersRow, error) {
	args := m.Called(ctx); return args.Get(0).([]db.ListUsersRow), args.Error(1)
}
func (m *MockQuerier) UpdateUserPassword(ctx context.Context, arg db.UpdateUserPasswordParams) error { return m.Called(ctx, arg).Error(0) }
func (m *MockQuerier) UpsertSetting(ctx context.Context, arg db.UpsertSettingParams) error { return m.Called(ctx, arg).Error(0) }
func (m *MockQuerier) SetDeviceDisabled(ctx context.Context, arg db.SetDeviceDisabledParams) error { return m.Called(ctx, arg).Error(0) }
...
func (m *mockDialogSrv) ReadAck(req *sip.Request, tx sip.ServerTransaction) error { return m.Called(req, tx).Error(0) }
func (m *mockDialogSrv) ReadBye(req *sip.Request, tx sip.ServerTransaction) error { return m.Called(req, tx).Error(0) }
...
func (m *mockDialogCli) ReadBye(req *sip.Request, tx sip.ServerTransaction) error { return m.Called(req, tx).Error(0) }
...
func TestHandleInvite_DisabledDevice(t *testing.T) {
	...
	cid := sip.CallIDHeader("inv-1"); req.AppendHeader(&cid)
	tx := new(mockServerTx)
...
func TestHandleInvite_Timeout(t *testing.T) {
	...
	mockPush.On("Send", mock.Anything, "android", mock.Anything, mock.Anything, mock.Anything, mock.Anything).Return(nil)
	...
	cid := sip.CallIDHeader("inv-t"); req.AppendHeader(&cid)
	tx := new(mockServerTx)
	tx.On("Respond", mock.Anything).Return(nil)
...
func TestHandleInvite_UnknownDevice(t *testing.T) {
	...
	req.AppendHeader(&sip.ToHeader{Address: sip.Uri{User: "7337", Host: "pbx.com"}})
	req.AppendHeader(&sip.FromHeader{DisplayName: "A", Address: sip.Uri{User: "a", Host: "h"}})
	cid := sip.CallIDHeader("inv-u"); req.AppendHeader(&cid)
	tx := new(mockServerTx)
	tx.On("Respond", mock.Anything).Return(nil)
...
func TestHandleAck_Success(t *testing.T) {
	...
	cid := sip.CallIDHeader("ack-1"); req.AppendHeader(&cid)
	tx := new(mockServerTx)
	cm.handleAck(req, tx)
...
func TestHandleAck_Error(t *testing.T) {
	...
	cm := &CallManager{dialogSrv: mockSrv}
	req := sip.NewRequest(sip.ACK, sip.Uri{Host: "test"})
	cid := sip.CallIDHeader("ack-2"); req.AppendHeader(&cid)
	tx := new(mockServerTx)
	cm.handleAck(req, tx)
	mockSrv.AssertCalled(t, "ReadAck", mock.Anything, mock.Anything)
}
...
func TestHandleBye_ServerSide(t *testing.T) {
	mockSrv := new(mockDialogSrv); mockSrv.On("ReadBye", mock.Anything, mock.Anything).Return(nil)
	mockCli := new(mockDialogCli); mockCli.On("ReadBye", mock.Anything, mock.Anything).Return(nil)
	cm := &CallManager{
	...
	cid := sip.CallIDHeader("bye-1"); req.AppendHeader(&cid)
	tx := new(mockServerTx)
	cm.handleBye(req, tx)
...
func TestHandleBye_ClientSide(t *testing.T) {
	mockSrv := new(mockDialogSrv); mockSrv.On("ReadBye", mock.Anything, mock.Anything).Return(assert.AnError)
	mockCli := new(mockDialogCli); mockCli.On("ReadBye", mock.Anything, mock.Anything).Return(nil)
	cm := &CallManager{dialogSrv: mockSrv, dialogCli: mockCli}
	req := sip.NewRequest(sip.BYE, sip.Uri{Host: "test"})
	cid := sip.CallIDHeader("bye-2"); req.AppendHeader(&cid)
	tx := new(mockServerTx)
	cm.handleBye(req, tx)
...
func TestHandleBye_UnknownDialog(t *testing.T) {
	mockSrv := new(mockDialogSrv); mockSrv.On("ReadBye", mock.Anything, mock.Anything).Return(assert.AnError)
	mockCli := new(mockDialogCli); mockCli.On("ReadBye", mock.Anything, mock.Anything).Return(assert.AnError)
	cm := &CallManager{dialogSrv: mockSrv, dialogCli: mockCli}
	req := sip.NewRequest(sip.BYE, sip.Uri{Host: "test"})
	cid := sip.CallIDHeader("bye-3"); req.AppendHeader(&cid)
...
func TestHandleCancel_UnknownCall(t *testing.T) {
	...
	req := sip.NewRequest(sip.CANCEL, sip.Uri{Host: "test"})
	cid := sip.CallIDHeader("unknown"); req.AppendHeader(&cid)
	tx := new(mockServerTx); tx.On("Respond", mock.Anything).Return(nil)
	cm.handleCancel(req, tx)
	tx.AssertCalled(t, "Respond", mock.Anything)
}
...
func TestMatchDevice_FallbackToUpstreamUser(t *testing.T) {
	...
	devices := []db.Device{{B2buaSipUser: "john_a", DeviceID: "d1"}, {B2buaSipUser: "john_b", DeviceID: "d2"}}
...
func TestNew(t *testing.T) {
	...
	mockPush := new(MockPushSender); mockPush.On("OnDeadToken", mock.Anything).Return()
...
func TestCleanup(t *testing.T) {
	mockPush := new(MockPushSender); mockPush.On("CancelPush", "call-1").Return()
	...
	cm.cleanup("call-1")
	cm.mu.RLock(); _, exists := cm.pending["call-1"]; cm.mu.RUnlock()
...
func TestIsBanned(t *testing.T) {
	...
	cm.banlistMu.Lock(); cm.banlist["1.2.3.4"] = time.Now().Add(time.Hour); cm.banlistMu.Unlock()
...
func TestRecordReject(t *testing.T) {
	...
	cm.failMu.Lock(); _, exists := cm.failCounts["127.0.0.1"]; cm.failMu.Unlock()
	assert.False(t, exists)
	cm.recordReject("1.2.3.4")
	cm.failMu.Lock(); assert.Equal(t, 1, cm.failCounts["1.2.3.4"]); cm.failMu.Unlock()
}

func TestClearFailures(t *testing.T) {
	cm := &CallManager{failCounts: map[string]int{"1.2.3.4": 7}}
	cm.clearFailures("1.2.3.4")
	cm.failMu.Lock(); _, exists := cm.failCounts["1.2.3.4"]; cm.failMu.Unlock()
	assert.False(t, exists)
}
...
func TestPruneStaleState(t *testing.T) {
	...
	cm.banlistMu.Lock(); cm.banlist["expired"] = time.Now().Add(-time.Second); cm.banlistMu.Unlock()
	cm.failMu.Lock(); cm.failCounts["at_threshold"] = maxRejectsBeforeBan; cm.failMu.Unlock()
	cm.pruneStaleState()
	cm.rejectThrottleMu.Lock(); _, exists := cm.rejectThrottle["stale"]; cm.rejectThrottleMu.Unlock()
	assert.False(t, exists)
	cm.banlistMu.Lock(); _, exists = cm.banlist["expired"]; cm.banlistMu.Unlock()
	assert.False(t, exists)
	cm.failMu.Lock(); _, exists = cm.failCounts["at_threshold"]; cm.failMu.Unlock()
	assert.False(t, exists)
}
...
func TestRemoveDeviceSource(t *testing.T) {
	...
	cm.mu.RLock(); _, exists := cm.deviceSource["u1"]; cm.mu.RUnlock()
	assert.False(t, exists)
}
...
func TestUpdateDeviceFromContact(t *testing.T) {
	...
	cp := sip.NewParams(); cp.Add("pn-provider", "a"); cp.Add("pn-param", "b"); cp.Add("pn-prid", "c")
