...
func NewTlsProtocol(
	...
	p.dial = func(addr *net.TCPAddr) (net.Conn, error) {
		// InsecureSkipVerify is required because gosip resolves the hostname to an IP
		// address before dialing, causing Go's TLS hostname verification to fail when
		// the certificate covers a DNS name but not an IP SAN.
